...
from fastapi.responses import JavaScriptResponse, HTMLResponse
...
OFFER = {
    "name": "AI Sales Assistant for Website",
    "price": "$99/month",
    "promise": "answers visitors, qualifies leads, and helps close sales automatically"
}
...
def wants_to_pay(message):
    m = message.lower()
    return any(x in m for x in [
        "buy", "pay", "payment", "checkout", "start now",
        "i want it", "sign me up", "order",
        "kupi", "kupit", "oplata", "oplatit", "hochu kupit",
        "beru", "gde oplata"
    ])
...
def wants_booking(message):
    m = message.lower()
    return any(x in m for x in [
        "book", "call", "demo", "meeting", "appointment",
        "schedule", "consultation",
        "zapis", "zapisat", "priem", "sozvon",
        "vstrecha", "demo", "konsultaciya"
    ])
...
def save_booking(email, phone, message, page_url, session_id):
    booking = {
        ...
        "confirmation": "Your booking request is confirmed. Our team will contact you shortly."
    }
...
def sales_reply(req):
    ...
    if wants_to_pay(message):
        save_lead(email, phone, message, req.page_url, req.session_id, "payment_link_sent")

        return (
            f"Great. You can start here: {PAYMENT_LINK}\n\n"
            f"Plan: {OFFER['name']} — {OFFER['price']}.\n"
    ...
    if wants_booking(message):
        if email or phone:
            ...
            return (
                "Perfect — your request is confirmed.\n\n"
                "Our team will contact you shortly to confirm the exact time.\n"
                f"You can also book directly here: {CALENDLY_LINK}"
            )

        return (
            "Sure — I can book you in. Please send your email or phone number, "
            "and I’ll confirm the request."
        )

    if wants_price(message):
        return (
            f"The starting price is {OFFER['price']}.\n\n"
            f"It includes: {OFFER['promise']}.\n\n"
            f"If you want to start now, here is the payment link: {PAYMENT_LINK}\n"
            f"Or book a quick demo here: {CALENDLY_LINK}"
        )
    ...
    if email or phone:
        ...
        return (
            ...
            f"Demo: {CALENDLY_LINK}"
        )

    if any(x in m for x in ["hello", "hi", "hey", "privet", "zdravstvuite"]):
        return (
            f"Hi! I’m the {COMPANY_NAME}.\n\n"
            "I can help you increase website sales by answering visitors instantly, "
            "capturing leads, and sending them to payment or booking.\n\n"
            "Do you want pricing, demo, or payment link?"
        )

    return (
        "Got it. The assistant can help you close more website visitors into buyers.\n\n"
        "I can do 2 things for you now:\n"
...
def home():
    return {
        ...
        "widget": '<script src="http://127.0.0.1:8001/widget.js"></script>'
    }
...
(function () {
  ...
  box.innerHTML = `
    ...
    <div id="ai-sales-window">
      ...
      <div id="ai-sales-messages">
        <div class="bot">
          Hi! I can help you get started, book a demo, or send you the payment link.
        </div>
      </div>
  ...
  quickButtons.forEach(btn => {
    btn.onclick = () => sendMessage(btn.getAttribute("data-text"));
  });
})();
"""
    return JavaScriptResponse(js)
...
<body>
  <div class="card">
    ...
    <p>The bot can answer questions, capture leads, send a payment link, or book a demo.</p>
